# STATE — Where We Are Right Now

> **Read this first, every session. Update it last, every session.**

**Last updated:** 2026-08-10 · **Phase:** Pre-production · **Warning level:** 🟢 GREEN

---

## Countdown

| | |
|---|---|
| **Deadline** | Aug 31, 2026 · 11:59 PM PT |
| **Days remaining** | **21** |
| Festival project opens | **TODAY, Aug 10** |
| Next gate | **GATE 1 — concept locked + compliance cleared, Aug 12 (T-2)** |

---

## Current status

| Workstream | Owner | Status |
|---|---|---|
| Contest intelligence | Catmull | ✅ Dossier complete, 11 sources |
| Council established | — | ✅ All five elements live in `.claude/agents/` |
| Memory + soul system | — | ✅ Operational |
| Scoring strategy | Braintrust | ✅ Rubric decoded, exploits identified |
| Concepts | Edwin | 🟡 Three drafted — **not yet ratified** |
| Reference study | Braintrust | ✅ Delivered |
| Production plan | Catmull | ✅ 23-day schedule set |
| The Void Room (app) | — | ✅ Built and **published** — see below |
| Early-warning system | Catmull | 🟡 Scripts + cron live; **Routines not yet created** |
| Script | Edwin | ⬜ Not started |
| Character refs | Phedon | ⬜ Not started |
| Sound / music | Anderson | ⬜ Not started |
| Campaign | Anderson | 🔴 **Not started — should already be running** |
| Generation | Phedon | 🔴 **Blocked on credits** |

---

## 🔴 Open blockers

1. **MCP credit balance = 0** (plan: Ultra). Gates every generation.
   Fix: enable auto-refill or top up — https://higgsfield.ai/mcp-credits?show_modal=auto_refill&source=mcp
   Best one-time rate: 4,000 credits / $190. Also check whether Ultra web credits
   exist separately from the MCP balance.
2. **`higgsfield.ai` is egress-blocked** in this environment. All contest facts were
   reconstructed via search. See `docs/00-verification-queue.md`.
3. **Campaign has not started.** 30% of the rubric is engagement and the clock is
   already running. Every silent day is forfeited score.

---

## Published

**The Void Room** — the whole project as one shareable page:
https://claude.ai/code/artifact/6cc9a223-2104-418d-9ac3-de09167d3f07

Private until shared from the page's share menu. Rooms link directly by hash:
`#mission` · `#council` · `#library` · `#theater`.

Source is `app/index.html`. After editing it, run `bash scripts/build-artifact.sh`
and re-publish **to the same URL** — do not mint a new one, the link is shared.

---

## Reminders

Three layers, documented in `docs/09-reminders.md`:

1. `bash scripts/standup.sh` — on demand, always works
2. `bash scripts/install-reminders.sh --install` — cron alerts on every gate
3. **Claude Routines — NOT YET CREATED.** They need interactive approval, which the
   headless session that built this could not give. In an interactive session, say
   *"create the festival Routines from docs/09-reminders.md"* — the specs and prompts
   are written and ready.

---

## Decisions pending

- [ ] **Which concept is primary?** Council now recommends **C · *Tidewalker*** —
      revised after the third juror (Paul W. S. Anderson) was confirmed. It is the only
      concept scoring ★★★★+ with all three judges. *Understudy* drops to secondary.
      Needs: Catmull compliance gate → Edwin spine test → Anderson propulsion test →
      Braintrust red team → ratify.
- [ ] **Portfolio or single film?** Entries are unlimited; 14 prizes exist.
- [ ] **Do we enter *Make Your Action Scene*** ($500K, same deadline)?
- [ ] Credit budget ceiling.

---

## Next actions

1. **Unblock credits.** Nothing generates until this is done.
2. **Ratify the concept.** Full council review of `docs/06-concepts.md`.
3. **Start the campaign today.** Anderson posts the process from day one.
4. **Aug 10:** enter the official project, confirm real submission mechanics, update
   the dossier immediately.
5. **Braintrust:** mine the Hell Grind breakdown for the character-consistency workflow.
