...
| **`design/continuity-audit.json`** | The 54 web renders of 13 Sep read from the account and matched to shots: which element each attached, which lane it came from, whether a frame was used (none were), and the identity breaks that fall out of that — read from metadata, not from watching the clips. |
| **`design/recreate-prompts.md`** | The seven shots to regenerate for continuity, each with what broke, the fix, and the corrected A-lane prompt ready to paste; plus the selections that fix three more shots without spending anything. |
